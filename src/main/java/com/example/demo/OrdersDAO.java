...
public class OrdersDAO {
	...
	public List<Orders> showVendorOrders(int ven_id) {
		String cmd = "select * from Orders where ven_id=? order by ord_id" ;
		List<Orders> ordersList=null;
		ordersList=jdbc.query(cmd,new Object[] {ven_id}, new RowMapper<Orders>() {
	 ...
	 public Orders[] customerorder(int cus_id) {
			String cmd = "select * from Orders where cus_id=? order by ord_id";
			List<Orders> c=null;
		...
		public List<Orders> showCustomerPendingOrders(int cus_id) {
			String cmd = "select * from Orders where  cus_id=? AND ORD_STATUS='PENDING' order by ord_id  ";
			List<Orders> ordersList=null;
